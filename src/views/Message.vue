<template>
  <div id="mesag" class="flexv wrap">
    ...
    <div class="flex center title">
      <span class="flex center read" :class="{'elect': type === 'normal'}" @click="changeTab(0, 'normal')">用户咨询留言</span>
      <span class="flex center share" :class="{'elect': type === 'family'}"
            @click="changeTab(1, 'family')">家庭保障留言</span>
    </div>
    <div class="flexv center ordinary" v-if="!is_member && type ==='family'">
      <h2 class="tit">您还未开通正式会员</h2>
      <p class="s-tit">开通后才可以体验"家庭保障留言"功能</p>
      <router-link to="/open_member" class="flex center btn">去开通</router-link>
    </div>
    <mescroll-vue
    ...
    >
      <div class="flexitemv mainbox">
        <div class="date listbox" :id="'dataList' + i">
          <div class="flexv centerv lists" v-for="(citem, cindex) of tab[i].list">
            <div class="flex top">
              <div class="headimg">
            ...
            <router-link :to="'/message/' + citem.message_id + '/' + type" class="flex center">点击查看留言详情</router-link>
          </div>
        </div>
      </div>
    </mescroll-vue>
