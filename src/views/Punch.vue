<template>
  <div class="flexitemv scroll" style="position: relative;">
    <div class="banner">
      <img src="../assets/image/banner.png" alt="" width="100%">
    </div>
    ...
    <div class="rank-box">
      <div class="flex rank-box-tab">
        <a href="javascript:" class="flexitemv center rank-box-tab__item" :class="{'rank-box-tab__item--active': rank.active === 1}" @click="selectActive(1)">
        ...
        <a href="javascript:" class="flexitemv center rank-box-tab__item" :class="{'rank-box-tab__item--active': rank.active === 2}" @click="selectActive(2)">
          <i class="rank-box-tab__icon iconfont icon-crown"></i> 我的排名 <span class="rank-box-tab__underline"></span>
        </a>
      </div>
      <ul class="area-rank js-rank" v-show="rank.active_province">
        <li class="flexv center rank-empty">
          <div class="show-punch-list" v-show="rank.get_province_data" @click="getProvincePunchUsers">点击查看更多排名</div>
        </li>
        <li class="flex centerv area-rank__item" v-if="rank.province.length > 0">
          <i class="flex center area-rank__icon iconfont icon-cup"></i>
          <div class="img area-rank__img lazy" :style="'background-image: url(' + rank.province[0].user.avatar +')'"></div>
          <div class="area-rank__name"><p>{{rank.province[0].user.nickname}}</p></div>
        ...
        <li class="flex centerv area-rank__item" v-show="rank.province.length" v-for="(item, index) of rank.province" :key="index">
          <i class="flex center area-rank__icon iconfont icon-badge"></i>
          <div class="img area-rank__img lazy" :style="'background-image: url(' + item.user.avatar +')'"></div>
          <div class="area-rank__name"><p>{{item.user.nickname}}</p></div>
      ...
      <div class="total-rank-box js-rank" v-show="rank.active_all">
        <div class="flexv center rank-empty">
          <div class="show-punch-list" v-show="rank.get_all_data" @click="getAllPunchUsers">点击查看更多排名</div>
        </div>
        <table class="total-rank">
          <tbody>
          <tr class="total-rank__tr" v-if="rank.current_user">
            <td class="total-rank__th-rank total-rank__num"><p>{{rank.current_user.rank}}</p></td>
            <td class="total-rank__th-user">
              <div class="flex centerv total-rank__content">
                <div class="img total-rank__img lazy" :style="'background-image: url('+ rank.current_user.user.avatar +');'"></div>
          ...
          <tr class="total-rank__tr" v-for="(item, index) of rank.all" :key="index">
            <td class="total-rank__num" v-if="index + 1 === 1"><p class="total-rank__first">{{index + 1}}</p></td>
            <td class="total-rank__num" v-else-if="index + 1 === 2"><p class="total-rank__second">{{index + 1}}</p></td>
            <td class="total-rank__num" v-else-if="index + 1 === 3"><p class="total-rank__third">{{index + 1}}</p></td>
            <td class="total-rank__num" v-else>{{index + 1}}</td>
            <td class="total-rank__info">
              <div class="flex centerv total-rank__content">
                <div class="img total-rank__img lazy" :style="'background-image: url('+ item.user.avatar +');'"></div>
                <div class="flexitemv total-rank__user">
        ...
        <button class="load-more" type="button" style="color: rgb(204, 204, 204);" v-if="rank.all.length > 0">只显示全国前100名</button>
      </div>
    </div>
    ...
    <div class="layer layer-success" v-show="punch_alert">
      <div class="layer__content layer-success__content">
        <img src="https://image.yetingjk.com/images/punch/success.png" width="100%">
...
<script>
  import {punchIndex, punchStore, UpdateUserInfo, punchProvince, punchTopHundred, punchCurrentUser} from '../api.js'
  ...
  export default {
    ...
    methods: {
      ...
      close_punch () {    //关闭打卡成功弹窗
        this.punch_alert = false
      },
      close_subscribe () {    //关闭关注公众号弹窗
        this.subscribe = true
      },
      selectActive (select) {     //切换排行榜状态
      ...
      getLocation () {    //更新用户省份
        ...
        geolocation.getCurrentPosition(function (r) {
          if (this.getStatus() == BMAP_STATUS_SUCCESS) {
            UpdateUserInfo({province: r.address.province}).then(function (res) {
