  ...
  <div class="flex center perfect-information">
    ...
    <div class='content'>
      ...
      <template v-else-if="type === 'poster'">
        <i class="flex center bls bls-cuo cuo" @click="cancelAlert"></i>
        <h3 class="flex center title">修改个人信息</h3>
      </template>
      <div class="flex center input">
        <span class="flex centerv">姓名</span>
        <input type="text" class="flexitem" :value="user.nickname" ref="nickname">
      </div>
      <div class="flex center input">
        <span class="flex centerv">手机号</span>
        <input type="text" class="flexitem" :value="user.phone" oninput="if(value.length>11)value=value.slice(0,11)"
               ref="phone">
      </div>
      <div class="flex center input" v-if="type === 'article_detail'">
        <span class="flex centerv">微信</span>
        <input type="text" class="flexitem" value="" ref="wechat">
      </div>
      <a href="javascript:;" class="flex center button" @click="perfect">保存修改</a>
    </div>
  </div>
</template>
