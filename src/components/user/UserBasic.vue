<template>
  <div id="basic" class="flexv wrap">
      ...
      <div class="flexitemv user">
        <div class="users">
          <div class="item portrait">
            ...
            <div class="flex centerv right">
              ...
              <input type="hidden" ref="avatar" :value="image_url.avatar">
              <input type="file" id="qrcode" :value="imgFile" style="position:absolute; clip:rect(0 0 0 0);"
                     accept="image/png, image/jpeg, image/gif, image/jpg" @change="uploadImg($event, 'avatar')">
          ...
          <div class="item name">
            ...
            <div class="flex centerv right">
              <input type="text" ref="nickname" class="flexi center userimg" :value="user.nickname" maxlength="11">
              <i class="flex center bls bls-yjt"></i>
            </div>
          </div>
          <div class="item phone">
            ...
            <div class="flex centerv right">
              <input type="tel" ref="phone" class="flex center userimg" :value="user.phone">
              <i class="flex center bls bls-yjt"></i>
            </div>
          </div>
          <div class="item phone">
            ...
            <div class="flex centerv right">
              <input type="text" ref="wechat" class="flex center userimg" :value="user.wechat">
              <i class="flex center bls bls-yjt"></i>
            </div>
          </div>
        </div>
        <div class="users">
          <div class="item area">
            ...
            <div class="flex centerv right">
              <input ref="profession" class="flex center userimg" type="text" :value="user.profession">
              <i class="flex center bls bls-yjt"></i>
            </div>
          </div>
          <div class="item area">
            ...
            <div class="flex centerv right">
              <input ref="employed_area" class="flex center userimg" type="text" placeholder="请选择"
                     :value="user.employed_area" @click="showPacker">
              <i class="flex center bls bls-yjt"></i>
            </div>
          </div>
        </div>
        <div class="users">
          <div class="item qrcode">
            ...
            <div class="flex centerv right">
              ...
              <input type="hidden" ref="qrcode" :value="image_url.qrcode">
              <input type="file" id="avatar" :value="imgFile" style="position:absolute; clip:rect(0 0 0 0);"
                     accept="image/png, image/jpeg, image/gif, image/jpg" @change="uploadImg($event, 'qrcode')">
              <i class="flex center bls bls-yjt"></i>
            </div>
          </div>
        </div>
        <router-link to="/index" class='clook'>查看如何获取二维码？</router-link>
        <div class="button">
          <a class="flex center" @click="updateSubmit">保存</a>
        </div>
      </div>
    </form>
    <upload-image
    ...
    <animation>
      <div class="city-picker" v-show="is_picker_show">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      confirm() {
        this.is_picker_show = !this.is_picker_show
        this.user.employed_area = this.myAddressProvince + ' ' + this.myAddressCity + ' ' + this.myAddresscounty
      },
      //提交表单
      updateSubmit() {
        let _this = this
...
<style lang="less" scoped>
  ...
  .city-picker {
    ...
    .picker-head {
      ...
      font-size: 1.2rem;
      background: #fff;
      .picker-head-left, .picker-head-center, .picker-head-right {
        float: left;
        width: 33.33%;
        text-align: center;
      }
      .picker-head-left, .picker-head-right {
        color: #3b8adf;
      }
      .picker-head-center {
        font-size: 1.4rem;
