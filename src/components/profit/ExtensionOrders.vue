    ...
    <div id="pay" class="flexv mainbox wrap box">
        <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
            <ul class="flexv reg-container" id="listbox">
                <li class="around" v-for="(item, index) of dataList" :key="index">
                    <img class="flex u-avatar" :src="item.user.avatar">
                    <div class="flexitemv reg-info">
                        <div class="flex centerv reg-title">您邀请的: <strong class="flexv centerv">{{item.user.nickname}}</strong> 付款成功</div>
                        <div class="between reg-date">
                            <div class="date">{{item.pay_at}}</div>
                            <div class="pay-num">&yen;{{item.price}}</div>
                        </div>
                    </div>
                </li>
            </ul>
...
<script>
    ...
    import { extensionOrders } from '../../api.js'
    ...
    export default {
        ...
        methods: {
            ...
            upCallback (page, mescroll) {
                // 联网请求
                let data = extensionOrders({page: page.num})
                data.then((response) => {
                    // 请求的列表数据
