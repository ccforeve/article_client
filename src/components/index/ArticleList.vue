...
<script>
  ...
  export default {
    ...
    methods: {
      search() {
        this.$router.push({path: 'article_search', query: {search_key: this.$refs.search_key.value}})
      },
      ...
      upCallback(page, mescroll) {
        ...
        _this.getListDataFromNet(mescroll.tabType, page.num, (curPageData) => {
          ...
          _this.tab[mescroll.tabType].list = _this.tab[mescroll.tabType].list.concat(curPageData.data); // 追加新数据
        }, () => {
      ...
      getListDataFromNet(tabType, pageNum, successCallback, errorCallback) {
        try {
          let res = getIndexArticleList({category_id: this.category_id, page: pageNum})
          res.then((res) => {
